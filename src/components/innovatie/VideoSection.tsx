...
const VideoSection = () => {
  const [isSupabaseVideoVisible, setIsSupabaseVideoVisible] = useState(false);
  const [isGoogleDriveVideoVisible, setIsGoogleDriveVideoVisible] = useState(false);
  const supabaseVideoRef = useRef<HTMLDivElement>(null);
  const googleDriveVideoRef = useRef<HTMLDivElement>(null);
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          if (entry.target === supabaseVideoRef.current) {
            setIsSupabaseVideoVisible(true);
          } else if (entry.target === googleDriveVideoRef.current) {
            setIsGoogleDriveVideoVisible(true);
          }
        }
      });
    }, observerOptions);

    if (supabaseVideoRef.current) {
      observer.observe(supabaseVideoRef.current);
    }
    if (googleDriveVideoRef.current) {
      observer.observe(googleDriveVideoRef.current);
    }
    ...
    <section className="py-16 md:py-24 bg-gradient-to-br from-background via-background/80 to-primary/5">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 max-w-7xl mx-auto">
          {/* Supabase Video */}
          <div className="group relative" ref={supabaseVideoRef}>
          ...
          {/* Google Drive Video */}
          <div className="group relative" ref={googleDriveVideoRef}>
            <div className="relative overflow-hidden rounded-2xl bg-card shadow-xl transition-all duration-300 hover:shadow-2xl hover:scale-[1.02]">
              <div className="aspect-video relative bg-gradient-to-br from-primary/10 to-primary/5">
                {isGoogleDriveVideoVisible ? (
                  <iframe
