  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
      {benefits.map((benefit, index) => (
        <Card key={index} className="group hover:shadow-xl transition-all duration-500 hover:-translate-y-2 border-0 bg-card/80 backdrop-blur-sm min-h-[180px] flex">
          <CardContent className="p-6 text-center flex-1 flex flex-col justify-center">
            ...
            <h5 className="font-display text-lg font-bold text-foreground mb-2 min-h-[28px] flex items-center justify-center">{benefit.title}</h5>
            <p className="text-muted-foreground leading-relaxed text-sm min-h-[40px] flex items-center justify-center">{benefit.description}</p>
          </CardContent>
        </Card>
      ))}
    </div>
